...
@AutoConfigureMockMvc // MockMvc를 자동으로 구성합니다.
...
public class ApiV1PostControllerTest {
    ...
    void t1() throws Exception {
        ...
        resultActions
                ...
                .andExpect(jsonPath("$.data.totalCount").value(postService.count()))
                .andExpect(jsonPath("$.data.post.id").value(post.getId()))
                .andExpect(jsonPath("$.data.post.createDate")
                        .value(Matchers.startsWith(post.getCreateDate().toString().substring(0, 20))))
                .andExpect(jsonPath("$.data.post.modifyDate")
                        .value(Matchers.startsWith(post.getModifyDate().toString().substring(0, 20))))
